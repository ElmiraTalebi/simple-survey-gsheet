...
def auto_answer_for_current_state(st) -> str:
    topic = st.session_state.current_topic
    question = normalize_text(st.session_state.current_question)
    topic_data = st.session_state.collected_data.get(topic, {})

    if topic == "pain":
        if "where" in question or "throat, tongue" in question:
            return topic_data.get("location", "throat")
        if "0–10" in st.session_state.current_question or "0-10" in question:
            return "7"
        if "constant" in question or "swallowing or eating" in question:
            return topic_data.get("timing", "constant")
        if "pain today" in question:
            return "Yes, I have throat pain."
        return "The pain is in my throat and is constant."

    if topic == "medications":
        return "Tylenol and oxycodone as needed."

    if topic == "nutrition":
        if "what are you able to eat" in question:
            return "Soft foods and shakes."
        if "difficult to eat or drink" in question:
            return "Pain when eating."
        if "drinking enough fluids" in question:
            return "Yes, I am drinking enough."
        if "nutritional shakes" in question:
            return "2 shakes per day."
        if "pain medication before meals" in question:
            return "Yes, sometimes before meals."
        return "Eating is reduced. I can manage soft foods and shakes because pain makes eating difficult."

    if topic == "weight":
        return "150 lbs"

    if topic == "oral_sores":
        return "No mouth sores."

    if topic == "dry_mouth":
        return "Yes, my mouth is dry all day."

    if topic == "mucus":
        return "Some thick mucus at night."

    if topic == "gi":
        return "No nausea, vomiting, or blood."

    if topic == "fatigue":
        return "Yes, I feel more tired than usual."

    if topic == "sleep":
        return "Not really, pain wakes me up."

    if topic == "swallowing":
        return "Yes, swallowing is painful but I can still drink liquids."

    if topic == "breathing":
        return "No breathing trouble."

    if topic == "mood":
        return "I feel anxious but supported."
...
def run_scenario(ns, st, answers: list[str]) -> dict[str, object]:
    ...
    idle_loops = 0
    while not st.session_state.finished and ns["has_remaining_topics"]() and idle_loops < 40:
        prior_section = st.session_state.selected_section
        move_to_next_section(ns, st)
        active_topic = ns["get_active_topic_for_section"](st.session_state.selected_section)
        if active_topic is None:
            idle_loops += 1
            continue
        if st.session_state.selected_section == prior_section and active_topic == st.session_state.current_topic:
            ns["process_turn"](auto_answer_for_current_state(st))
            idle_loops += 1
        else:
            idle_loops = 0
