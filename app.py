...
_REPLY_SUGGESTION_SYS = """
You create short suggested patient replies for a clinical chatbot.
...
Rules:
  ...
  - Suggestions are example patient replies, not assistant questions.
  - Keep each suggestion short, natural, and tap-friendly.
  - For free-text questions, write short first-person answer fragments the patient could plausibly tap.
  - For number questions, return only numeric strings within the valid range if provided.
  - Do not invent alarming symptoms the patient has not mentioned.
  - It is okay to include one uncertainty option like "Not sure" when appropriate.
  - Avoid duplicate meaning.
  - Use recent topic context so the suggestions fit the current branch.
  - Use last-visit context only if it helps orient a plausible answer.
...
def _dedupe_preserve(items: list[str], limit: int = 4) -> list[str]:
    seen = set()
    deduped = []
    for item in items:
...
def _placeholder_examples(placeholder: str) -> list[str]:
    ...
    return _dedupe_preserve(parts, limit=3)


def _fallback_reply_suggestions(step: dict, last_visit_same_question_answer: Any = None) -> list[str]:
    stype = step.get("type")
    ...
    if stype == "number":
        ...
        weighted = [str(min_v), str(mid), str(max_v)]
        if min_v == 0 and max_v == 10:
            weighted = ["0", "5", "8"]
        return _dedupe_preserve(weighted, limit=3)

    suggestions = []

    last_answer = str(last_visit_same_question_answer or "").strip()
    if last_answer and len(last_answer) <= 28:
        suggestions.append(last_answer)

    if "how often" in text or "frequency" in text:
        suggestions.extend(["A few times a day", "Once a day", "Off and on"])
    elif "when did" in text or "when" in text and "start" in text:
        suggestions.extend(["A few days ago", "About a week ago", "Not sure exactly"])
    elif "where" in text:
        suggestions.extend(["In my throat", "On my tongue", "Near my jaw"])
    elif "what are you able to eat" in text:
        suggestions.extend(["Mostly soft foods", "Mostly liquids", "Very little"])
    elif "what are you using" in text or "what are you doing" in text:
        suggestions.extend(["Nothing right now", "Medication helps a little", "Still figuring it out"])
    elif "who is supporting you" in text:
        suggestions.extend(["My spouse", "My family", "A friend helps"])
    elif "what kind of support" in text:
        suggestions.extend(["Help with transportation", "More emotional support", "More information"])

    suggestions.extend(_placeholder_examples(placeholder))

    if stype == "free_text":
        suggestions.append("Not sure")

    return _dedupe_preserve(suggestions, limit=4)


def run_reply_suggestion_agent(
    step: dict,
    topic_history: list[dict[str, str]],
    recent_questions: list[str],
    last_visit_same_question_answer: Any = None,
    ...
    default = {
        "suggestions": _fallback_reply_suggestions(step, last_visit_same_question_answer),
    }
    if not openai_client:
        return default
    ...
    if step.get("type") == "number":
        payload["min_v"] = step.get("min_v")
        payload["max_v"] = step.get("max_v")

    result = _call_agent(_REPLY_SUGGESTION_SYS, payload, max_tokens=180)
    suggestions = result.get("suggestions") if isinstance(result, dict) else None
    if not isinstance(suggestions, list):
        return default

    cleaned = []
    min_v = int(step.get("min_v", 0)) if step.get("type") == "number" else None
    max_v = int(step.get("max_v", 0)) if step.get("type") == "number" else None
    for item in suggestions:
        ...
        if step.get("type") == "number":
            ...
            if numeric < min_v or numeric > max_v:
                continue
            cleaned.append(str(numeric))
        else:
            cleaned.append(text[:40].strip())

    cleaned = _dedupe_preserve(cleaned, limit=4)
    return {"suggestions": cleaned or default["suggestions"]}


def _suggested_replies_for_step(
    topic_key: str,
    step: dict,
    ...
    state = state or st.session_state.topic_states.get(topic_key, {})
    cache = state.setdefault("generated_reply_suggestions", {})
    resolved_key = cache_key or step.get("id")
    cached = cache.get(resolved_key)
    if isinstance(cached, list) and cached:
        return cached

    last_answer = st.session_state.get("last_checkin", {}).get(topic_key, {}).get(step.get("id"))
    result = run_reply_suggestion_agent(
        step,
        topic_history=_recent_topic_history(state),
        recent_questions=_recent_topic_questions(state),
        last_visit_same_question_answer=last_answer,
    )
    suggestions = _dedupe_preserve(result.get("suggestions", []), limit=4)
    cache[resolved_key] = suggestions
    return suggestions
...
def _render_suggested_reply_buttons(
    ...
    suggestions = _dedupe_preserve(suggestions, limit=4)
    if not suggestions:
        return

    st.markdown(
        '<div style="font-size:11px;color:#73879a;font-weight:700;letter-spacing:0.04em;'
        'text-transform:uppercase;margin:8px 4px 6px 4px;">Suggested replies</div>',
        unsafe_allow_html=True,
    )
...
def render_input(topic_key: str, step: dict, prev_answer=None):
    ...
    # ── Number ───────────────────────────────────────────────────
        ...
        suggestions = _suggested_replies_for_step(topic_key, step, state)
        user_text = st.text_input(
            "Reply",
            key=text_key,
            label_visibility="collapsed",
        ...
        _render_suggested_reply_buttons(
            ...
            step,
            suggestions,
            key_prefix="suggested_reply",
            on_select=lambda suggestion: handle_answer(
                topic_key,
                step,
                int(float(suggestion)),
                source="typed",
    ...
    # ── Free text ────────────────────────────────────────────────
        ...
        _render_suggested_reply_buttons(
            ...
            suggestions,
            key_prefix="suggested_reply",
            on_select=lambda suggestion: handle_answer(topic_key, step, suggestion, source="free_text"),
        )
        with st.container():
...
def render_topic_detail(topic_label: str, topic_key: str):
    ...
    if state.get("waiting_for_followup"):
        ...
        _render_suggested_reply_buttons(
            ...
            followup_step,
            pending_suggestions,
            key_prefix="suggested_followup_reply",
            on_select=lambda suggestion: handle_pending_followup(topic_key, suggestion, source="suggested"),
        )
        with st.container():
